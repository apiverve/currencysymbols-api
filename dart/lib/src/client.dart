...
class CurrencysymbolsValidationException implements Exception {
  final List<String> errors;

  CurrencysymbolsValidationException(this.errors);

  @override
  String toString() => 'CurrencysymbolsValidationException: ${errors.join("; ")}';
}
...
/// Currency Symbols API client.
///
/// For more information, visit: https://apiverve.com/marketplace/currencysymbols?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [currency] (required) - The currency code for which you want to get the symbol
class CurrencysymbolsClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'currency': ValidationRule(type: 'string', required: true),
  };

  CurrencysymbolsClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/currencysymbols',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [CurrencysymbolsValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw CurrencysymbolsValidationException(errors);
    }
  }

  /// Execute a request to the Currency Symbols API.
  ///
  /// Parameters are validated before sending the request.
  Future<CurrencysymbolsResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw CurrencysymbolsException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw CurrencysymbolsException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw CurrencysymbolsException('Resource not found');
      } else {
        throw CurrencysymbolsException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is CurrencysymbolsException) rethrow;
      throw CurrencysymbolsException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Currency Symbols API client.
class CurrencysymbolsException implements Exception {
  final String message;

  CurrencysymbolsException(this.message);

  @override
  String toString() => 'CurrencysymbolsException: $message';
}
